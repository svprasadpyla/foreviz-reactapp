...
import { deleteInventoryService, getInventoryListService } from "../../services/inventory";
import _ from "lodash";
import { toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import PaginatedList from "../Shared/pagination";
import ManageTag from "../Inventory/ManageTag";
...
class PartDetails extends Component {
    ...
    handleTagData = async (payload) => {
        try {
            ...
            if (response && !_.isEmpty(response.data) && response.status.toLowerCase() === "success") {
                ...
                this.handleInventoryData({ "page": 0, "size": this.state.size, "filters": [] });
            }
        } catch (error) {
    ...
    handleTagFilter = (tagObj) => {
        this.handleInventoryData({ "page": 0, "size": this.state.size, "filters": tagObj.filters });
    }
    ...
    openAddPart = () => {
        this.setState({ addPart: true, manageTag: false, partDetails: false,selectedPart:{} });
    }
    ...
    handleEditPart = (partObj) => {
        this.setState({ addPart: true, manageTag: false, partDetails: false, selectedPart:partObj });
    }

    handleDeletePart = (partObj) => {
        this.handleDeletePartById({ "id": partObj.id });
    }

    handleDeletePartById = async (payload) => {
        try {
            const response = await deleteInventoryService(payload);
    ...
    render() {
        ...
        return (
            <>
                ...
                {
                    this.state.addPart && <AddNewPart onHide={() => this.closeAddPart()} selectedPartObj = {this.state.selectedPart}/>
                }

                {
                    ...
                    <div>
                        <div className="icon-container">
                            ...
                            <div className="icon-wrapper">
                                <img src="../assets/images/icons/icon-tag.svg" alt="Add Tag"
                                            ...
                                            <th key={column.accessor} style={{ position: "relative", padding: "10px" }}>
                                                ...
                                                {this.state.showPopup === column.accessor && (
                                                    <div ref={this.popupRef} className="filter-popup">
                                                        ...
                                                        {
                                                            column.accessor === "tag" && (
                                                                <select className="filter-input" onChange={(e) => {
                                                                    const selectedTag = this.state.manageTagList.find(tag => tag.tagName === e.target.value);
                                                ...
                                                }
                                                {
                                                    ...
                                                    >
                                                        ...
                                                        <div className="d-flex align-items-center justify-content-between mt-3">
                                                            ...
                                                            <button type="button" className="btn btn-xs btn-next" onClick={(e) => this.handleApplyFilter(e)}>Apply</button>
                                                        </div>
                                                ...
                                                if (column.accessor === 'actions') {
                                                    return (
                                                        <td className="actions-list">
                                                            <img src="../assets/images/foreviz/icon-edit.svg" alt="Edit" onClick={() => this.handleEditPart(row)} />
                                                            <img src="../assets/images/foreviz/icon-delete.svg" alt="Delete" onClick={() => this.handleDeletePart(row)} />
                                                        </td>
                                                    );
                                                }
                                                // Example of another condition
                                                else {
                                                    return (
                                                        <td key={column.accessor}>{row[column.accessor]}</td>
                                                    );
                ...
                {this.state.totalRecords > 10 &&
                    (
                        <div className='Onboard-pagenation multi-pagenation pagination-container mt-3'>
                            <PaginatedList
                                previousClassName="pagination-prev hidden"
                                nextClassName="pagination-next hidden"
                                breakLabel="..."
