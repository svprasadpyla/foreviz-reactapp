...
class InventoryList extends Component {
    ...
    setFilter = (columnId, value) => {
        ...
        //     filters: { ...prevState.filters, [columnId]: value }, selectedFilterKey : columnId, selectedFilterValue : value
        // }));
        this.setState({ selectedFilterKey: columnId, selectedFilterValue: (columnId === "partNumber" || columnId === "partName" || columnId === "customerId") ? value : Number(value) });
    };
    ...
    handleFileUpload = () => {
        this.setState({ showPartsUpload: true });
    };

    closeEmployeeModel = (response) => {
        this.setState({ showPartsUpload: false });
        //toast(response, { type: "success", autoClose: 2000, position: "bottom-right" });
    ...
    openManageTag = () => {
        this.setState({ manageTag: true });
    }

    closeManageTag = () => {
        this.setState({ manageTag: false });
    }
    ...
    render() {
        ...
        return (
            <>
                {
                    this.state.showPartsUpload && <UploadInventory show={this.state.showPartsUpload}
                        onHide={(response) => this.closeEmployeeModel(response)} />
                }
                {
                                    ...
                                    <th key={column.accessor} style={{ position: "relative", padding: "10px" }}>
                                        ...
                                        {this.state.showPopup === column.accessor && (
                                            <div ref={this.popupRef} className="filter-popup">
                                                ...
                                                {
                                                    (column.accessor === "partNumber" || column.accessor === "partName" || column.accessor === "customerId") && (
                                                        <input
                                                            type="text"
                                                ...
                                                {column.accessor === "lastPostingDate" && (
                                                    <div className="date-filter">
                                                        <input
                                                            ...
                                                            onChange={(e) => this.setFilter("startDate", e.target.value)}
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => this.setFilter("endDate", e.target.value)}
                ...
                {this.state.totalRecords > 10 && (
                    <div className='Onboard-pagenation multi-pagenation pagination-container'>
                        <PaginatedList
